...
class Languages extends Component {
    ...
    public $isLanguageModelEdit = false;
    ...
    public function mount() {
        ...
        $this->state['enable_browser_language_detection'] = config('app.settings.enable_browser_language_detection', true);
    }
    ...
    public function clearForm() {
        ...
        $this->isLanguageModelEdit = false;
    }
    ...
    public function addLanguage() {
        ...
        $is_active = $this->state['languages'][$this->form['language']]['is_active'] ?? false;
        $this->state['languages'][$this->form['language']] = [
            'label' => $this->form['label'],
            'type' => ($this->form['type'] == 'ltr' ? 'ltr' : 'rtl'),
            'is_active' => $is_active,
        ];
        if (!$this->isLanguageModelEdit) {
    ...
    public function deleteLanguage($language) {
        ...
        array_push($this->jsonToDelete, $language);
    }
    ...
    public function save() {
        $settings = Setting::whereIn('key', ['languages', 'language', 'language_in_url', 'enable_browser_language_detection'])->get();
        ...
        foreach ($this->jsonToAdd as $json) {
            ...
            if (!File::exists($path)) {
                ...
                $content = File::exists($enPath) ? File::get($enPath) : '{}';
                File::put($path, $content);
            }
        }
        foreach ($this->jsonToDelete as $json) {
            $path = base_path('lang/' . $json . '.json');
            if (File::exists($path)) {
                File::delete($path);
            }
