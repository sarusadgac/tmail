                    ...
                    <div class="flex items-baseline space-x-4">
                        @foreach($menus as $menu)
                            ...
                            <div @click.away="open = false" class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute left-0 w-full mt-2 origin-top-right shadow-lg md:w-48 z-10">
                                    <div class="px-2 py-2 bg-white shadow dark-mode:bg-gray-800">
                                        ...
                                        <a class="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent text-gray-700 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-100 focus:bg-gray-100 focus:outline-none" href="{{ $child->link }}" target="{{ $child->target }}">{!! __($child->name) !!}</a>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                            @else
                                ...
                                <a href="{{ $menu->link }}" class="px-3 py-2 text-sm font-semibold text-left bg-transparent text-gray-700 {{ url()->current() === $menu->link ? 'bg-gray-200' : '' }} md:w-auto md:inline hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none" target="{{ $menu->target }}">{!! __($menu->name) !!}</a>
                                @endif
                            @endif
                        @endforeach
                        @if(Auth::check() && Auth::user()->role == 7)
                            <a href="{{ route('admin') }}" class="px-3 py-2 text-sm font-semibold text-left bg-transparent border-dashed border-2 border-red-700 text-red-700 md:w-auto md:inline hover:bg-red-100">{{ __('Admin') }}</a>
                        @endif
                    </div>
                </div>
                <div class="flex items-center">
                        ...
                        <div class="relative">
                            <form action="{{ route('locale', '') }}" id="locale-form" method="post">
                                ...
                                <select class="block appearance-none bg-gray-200 cursor-pointer text-gray-800 py-1 focus:outline-none" name="locale" id="locale">
                                    ...
                                    @if(in_array($locale, config('app.settings.languages')))
                                    <option {{ (app()->getLocale() == $locale ) ? "selected" : "" }} value="{{ $locale }}">{{ isset(config('app.locales_text')[$index]) ? config('app.locales_text')[$index] : $locale }}</option>
                                    @endif
                                    @endforeach
                                </select>
                            </form>
                ...
                <div class="flex flex-col items-center justify-between">
                    <div class="flex flex-col items-center space-y-2">
                        @foreach($menus as $menu)
                            ...
                            <div @click.away="open = false" class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute left-0 w-full mt-2 origin-top-right shadow-lg md:w-48 z-10">
                                    <div class="px-2 py-2 text-center bg-white shadow dark-mode:bg-gray-800">
                                        @foreach($menu->getChild() as $child)
                                        <a class="block text-sm font-semibold bg-transparent text-gray-600 md:mt-0 focus:text-gray-900 focus:bg-gray-100 focus:outline-none" href="{{ $child->link }}" target="{{ $child->target }}">{!! __($child->name) !!}</a>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                            @else
                                @if($menu->parent_id === null)
                                <a href="{{ $menu->link }}" class="px-3 py-2 text-sm font-semibold text-left bg-transparent text-white {{ url()->current() === $menu->link ? 'bg-gray-200' : '' }} md:w-auto md:inline focus:text-gray-900 focus:bg-gray-200 focus:outline-none" target="{{ $menu->target }}">{!! __($menu->name) !!}</a>
                                @endif
                            @endif
                        @endforeach
                        @if(Auth::check() && Auth::user()->role == 7)
                            ...
                            <div class="relative">
                                <form action="{{ route('locale', '') }}" id="locale-form-mobile" method="post">
                                    ...
                                    <select class="block appearance-none bg-gray-200 cursor-pointer text-gray-800 py-1 focus:outline-none" name="locale" id="locale-mobile">
                                        ...
                                        <option {{ (app()->getLocale() == $locale ) ? "selected" : "" }} value="{{ $locale }}">{{ $locale }}</option>
                                        @endforeach
                                    </select>
                                </form>
                            </div>
